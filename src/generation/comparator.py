from __future__ import annotations

import os
import re

import structlog
from dotenv import load_dotenv
from google import genai

from src.embedding.embedder import Embedder
from src.embedding.qdrant_store import QdrantStore
from src.generation.gemini_utils import generate_with_fallback
from src.models.responses import Citation, CompareResponse
from src.models.schemas import RetrievedChunk

load_dotenv()
log = structlog.get_logger()

_SYSTEM_INSTRUCTION = """You are a senior financial analyst comparing two financial documents.
Provide a structured, objective comparison covering each requested topic.

Rules:
1. For every claim from Source A, cite as [Source A, Page: N]
2. For every claim from Source B, cite as [Source B, Page: N]
3. Use exact figures from the documents — do not round or paraphrase numbers
4. Be balanced: highlight both similarities and differences
5. If a topic is not addressed in one document, state that explicitly
6. You can answer in Dutch or English — match the language of the request
"""

# Separate unambiguous patterns for each source — no document-name guessing needed
_CITATION_A_RE = re.compile(r"\[Source A,\s*Page:\s*(?P<page>\d+)\s*\]", re.IGNORECASE)
_CITATION_B_RE = re.compile(r"\[Source B,\s*Page:\s*(?P<page>\d+)\s*\]", re.IGNORECASE)


class Comparator:
    """Compares two financial documents across specified topics using Gemini."""

    def __init__(
        self,
        embedder: Embedder | None = None,
        store: QdrantStore | None = None,
    ) -> None:
        self.embedder = embedder or Embedder()
        self.store = store or QdrantStore()
        self._gemini = genai.Client(api_key=os.getenv("GEMINI_API_KEY"))

    def compare(
        self,
        document_name_a: str,
        document_name_b: str,
        focus_topics: list[str],
    ) -> CompareResponse:
        """Retrieve relevant chunks from both documents and ask Gemini to compare."""
        combined_query = " ".join(focus_topics)
        query_vec = self.embedder.embed_query(combined_query)

        chunks_a = self.store.search(query_vec, top_k=8, document_filter=document_name_a)
        chunks_b = self.store.search(query_vec, top_k=8, document_filter=document_name_b)

        topics_md = "\n".join(f"- {t}" for t in focus_topics)
        user_prompt = (
            f"Compare the two financial documents on these topics:\n{topics_md}\n\n"
            f"=== Source A: {document_name_a} ===\n"
            f"{_build_context(chunks_a, 'Source A')}\n\n"
            f"=== Source B: {document_name_b} ===\n"
            f"{_build_context(chunks_b, 'Source B')}\n\n"
            "Provide a section for each topic. "
            "Cite Source A content as [Source A, Page: N] and "
            "Source B content as [Source B, Page: N]."
        )

        response, model_used = generate_with_fallback(
            self._gemini,
            contents=user_prompt,
            config=genai.types.GenerateContentConfig(
                system_instruction=_SYSTEM_INSTRUCTION,
                temperature=0.1,
            ),
        )

        text = response.text.strip()
        log.info(
            "comparison_generated",
            doc_a=document_name_a,
            doc_b=document_name_b,
            model=model_used,
        )

        return CompareResponse(
            document_name_a=document_name_a,
            document_name_b=document_name_b,
            comparison=text,
            citations_a=_extract_citations(text, document_name_a, chunks_a, _CITATION_A_RE),
            citations_b=_extract_citations(text, document_name_b, chunks_b, _CITATION_B_RE),
            model_used=model_used,
        )


# ------------------------------------------------------------------
# Helpers
# ------------------------------------------------------------------


def _build_context(chunks: list[RetrievedChunk], label: str) -> str:
    if not chunks:
        return "[No relevant content retrieved]"
    return "\n\n".join(
        f"[{label}, Page {c.page_number}]\n{c.text}" for c in chunks
    )


def _extract_citations(
    answer: str,
    doc_name: str,
    chunks: list[RetrievedChunk],
    pattern: re.Pattern,
) -> list[Citation]:
    seen: set[int] = set()
    citations: list[Citation] = []

    for match in pattern.finditer(answer):
        page_num = int(match.group("page"))
        if page_num in seen:
            continue
        seen.add(page_num)
        source = next(
            (c.text[:300] for c in chunks if c.page_number == page_num), ""
        )
        citations.append(
            Citation(document_name=doc_name, page_number=page_num, source_text=source)
        )

    return citations
